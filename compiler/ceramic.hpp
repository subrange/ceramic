...
#define CERAMIC_ALIGN(n) __declspec(align(n))
...
template <typename T> inline T ceramic_cimag(std::complex<T> const &c) {
    return std::imag(c);
}
} // namespace ceramic
...
inline long double ceramic_creal(const ceramic_cldouble &c) { return c.real(); }

inline float ceramic_cimag(const ceramic_cfloat &c) { return c.imag(); }
inline double ceramic_cimag(const ceramic_cdouble &c) { return c.imag(); }
inline long double ceramic_cimag(const ceramic_cldouble &c) { return c.imag(); }
} // namespace ceramic
